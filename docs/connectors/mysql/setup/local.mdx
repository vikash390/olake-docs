...
services:
  ...
  primary_mysql:
    ...
    command: >
      ...
      --log-bin=mysql-bin                 # Enable binary logging, which is needed for replication and CDC.
      --binlog-format=ROW                 # Use ROW format for binary logs to track every change in each row.
      --gtid-mode=ON                      # Enable Global Transaction Identifiers (GTIDs).
      --enforce-gtid-consistency=ON        # Ensure that all transactions are GTID compliant.
      --local-infile=1                    # Enable local data loading for importing files.
      --binlog_expire_logs_seconds=604800  # Set the binary log expiration time to 7 days (604800 seconds).
    ...
    healthcheck:
      test: ["CMD", "mysqladmin", "ping", "-h", "localhost", "-u", "root", "-ppassword"]  # Healthcheck command to ensure MySQL is responsive.
    ...
    entrypoint: >
      bash -c "echo 'Creating replication user...';
               ...
               mysql -h primary_mysql -P 3306 -u root -ppassword -e \"
                 ...
                 FLUSH PRIVILEGES;\";
               echo 'Replication user created.'"  # Output message to indicate success.
    networks:
  ...
  data-loader:
    ...
    entrypoint: >
      ...
      # Poll MySQL server until it responds to ping.
      ...
      # Create a sample table in the "main" database if it doesn't exist.
      ...
      # Insert a sample record into the sample_table.
      mysql -h primary_mysql -P 3306 -u root -ppassword main -e 'INSERT INTO sample_table (name) VALUES (\"sample_data\");'; \
      echo 'Data insertion complete!'"  # Inform that data loading is done.
...
For MySQL CDC, you need to ensure that the following configurations are set in your MySQL instance (`exec` into the container, and do `mysql -u root -p`. Refer below Troubleshooting section for more details):
...
Now that you are setup with a local database setup, head over to the [getting started guide](../../../getting-started/mysql#step-2-generate-a-catalog-file) to start syncing data with OLake.
