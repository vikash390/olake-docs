...
- **Note**: The `host.docker.internal` is used to refer to the host machine from within the Docker container. This is useful for connecting to services running on the host from within a container.
...
Now that you are setup with a local database setup, head over to the [getting started guide](../../../getting-started/mongodb#step-2-generate-a-catalog-file) to start syncing data with OLake.
