...
The following bash script automates the process of creating and starting the Docker Compose stack, initiating the replica set, and ingesting sample Reddit data. Save this as a shell script (for example, `setup.sh`).
...
docker exec mongo mongoimport --username admin --password password --authenticationDatabase admin --db reddit --collection funny --file /tmp/reddit.json --jsonArray --quiet
