...
`OLAKE_BASE_PATH="$HOME/PATH_TO_OLAKE_DIRECTORY/olake/drivers/mongodb/config" && \` maps to 
`OLAKE_BASE_PATH="/Users/JOHN_DOE_USERNAME/Desktop/projects/olake/drivers/mongodb/config" && \` in macOS and Linux systems. Follow the same pattern in other systems.
:::
