...
| **`partition_regex`** | string        | `"{now(),2025,YY}-{now(),2025,MM}-{now(),2025,DD}/{string_change_language,,}"` <br/> or <br/> `"{,1999,YY}-{,09,MM}-{,31,DD}/{latest_revision,,}"` | A pattern defining how to partition the data. Includes date tokens (e.g., year, month, day) or other markers like language or revision indicators. |

For more information about `partition_regex`, refer to [S3 partition documentation](/docs/writers/s3).
