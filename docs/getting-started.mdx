...
|---------------------|--------------------------------------------------------------------------------------------|-------------------------|
| `hosts`            | List of MongoDB hosts. Use DNS SRV if `srv` = `true`.                                      | `["host1:27017"]`      |
| `username/password`| Credentials for MongoDB authentication.                                                    | `"test"/"test"`        |
| `authdb`           | Authentication database (often `admin`).                                                   | `"admin"`              |
| `replica-set`      | Name of the replica set, if applicable.                                                    | `"rs0"`                |
| `read-preference`  | Which node to read from (e.g., `secondaryPreferred`).                                      | `"secondaryPreferred"` |
| `srv`              | If using DNS SRV connection strings, set to `true`.                                        | `true`                 |
| `server-ram`       | Memory management hint for the OLake container.                                            | `16`                   |
| `database`         | The MongoDB database name to replicate.                                                    | `"database"`           |
| `max_threads`      | Maximum parallel threads for chunk-based snapshotting.                                     | `50`                   |
| `default_mode`     | Default sync mode (`"cdc"` or `"full_refresh"`).                                           | `"cdc"`                |
