...
- **MongoDB** credentials (hosts, replica set name, username/password if applicable).  
  ...
  - [Docker Compose instructions to spin up MongoDB replica sets](../docs/connectors/mongodb/docker-compose)
  - Need sample dataset to ingest in MongoDB? Refer  -> [Sample Datasets](../docs/resources/dataset)
...
```json live title="olake_folder/writer.json"
{
  "type": "PARQUET",
  "writer": {
    "normalization": false,
...
Read more about S3 configuration in [S3 Writer Docs](./writers/s3)
...
Refer to [source configuration](../docs/configs/source) for more details on `config.json`.
...
- **`partition_regex`**: Specify the regex pattern. For more details, refer to [S3 docs](./writers/s3#s3-data-partitioning)
...
For more details on the  `state.json` configuration, refer the [state docs](../docs/configs/state)
