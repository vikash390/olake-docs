...
Welcome to **OLake** – the fastest open source DB-to-Data LakeHouse pipeline designed to bring your Database data into modern analytics ecosystems like Apache Iceberg. OLake was born out of the need to eliminate the toil of one-off ETL scripts, combat performance bottlenecks, and avoid vendor lock-in with a clean, high-performing solution.
...
- **Writers:** 
  - Tightly integrated with drivers, Writers (Apache Iceberg format) ensure that once data is extracted, it is immediately pushed to your chosen destination—whether that be local storage or cloud-based solutions.
