...
| `partition_strategy`                  |  The partition strategy for backfill               |   `timestamp`, uses `splitVector` strategy if the field is left empty. Refer [here](https://olake.io/blog/what-makes-olake-fast#mongodb) for more details.          | |
