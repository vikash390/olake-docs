...
### Local Minio + JDBC / REST catalog (Local Test Setup)
...
Refer [here](../iceberg/catalog/overview) for other catalog configs.
...
:::info
The `__op` column values will have `r`, `c`, `u` that stands for - "r" for read/backfill, "c" for create, "u" for update.
:::
