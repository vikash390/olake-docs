...
A sample disover command would look like this:
...
:::info
The `olakego/source-mongodb` is the OLake image for MongoDB source. You can replace it with the respective source image for PostgreSQL (source-postgres) or MySQL (source-mysql) .
:::

<TabItem value="glue" label="Using OLake Local Build" default>

```bash
./build.sh driver-mongodb sync --config /Users/priyansh_dz/Desktop/projects/olake/drivers/mongodb/config/config.json --catalog /Users/priyansh_dz/Desktop/projects/olake/drivers/mongodb/config/catalog.json --destination /Users/priyansh_dz/Desktop/projects/olake/drivers/mongodb/config/writer.json
```
