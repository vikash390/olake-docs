...
|----------------------------------------|---------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------|
| **gRPC Connection Fails**              | The gRPC server is not running or using an incorrect port.     | - Ensure the Java gRPC server is running.<br/>- Verify that the port in `writer.json` matches the server's port. |
| **Data Not Appearing in Spark**        | Misconfiguration or delayed data propagation.                 | - Double-check the `writer.json` configuration.<br/>- Connect to the `spark-iceberg` container and run a Spark SQL query.  |
