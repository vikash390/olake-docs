...
## Assumption: 
All services involved in the sync process — including the OLake source image, Nessie, MinIO, and any other dependencies — must be part of the same Docker network. Use container names (e.g., http://nessie:19120, http://minio:9000) for inter-service communication instead of localhost .
