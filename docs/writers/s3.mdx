...
:::info
1. The generated `.parquet` files use SNAPPY compression. Note that SNAPPY is no longer supported by S3 Select when performing queries.  
...
- **granularity:**  
  *(Optional)* For time-based columns, you can specify the granularity. Supported values include `HH` (hour), `DD` (day), `WW` (week), `MM` (month), and `YY` (year).
...
| `{now(),2025,YY}-{now(),2025,MM}` | Combines year and month from the current timestamp.                                                                                                                              | `2025-02`                    |
| `{now(),2025,YY}-{now(),2025,MM}-{now(),2025,DD}` | Combines year, month, and day from the current timestamp.                                                                                                         | `2025-02-13`, `2025-12-31`     |
