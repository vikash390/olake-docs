...
:::info
OLake UI is live (beta)! You can now use the UI to configure your MongoDB source, discover streams, and sync data. Check it out at [OLake UI](./olake-ui) regarding how to setup using Docker Compose and running it locally.
...
Refer to [MongoDB Connector documentation](../connectors/mongodb/overview) for more details.
:::


## TLDR:
1. **Create a [`source.json`](../connectors/mongodb/config)** with your MongoDB connection details.
...
- **MongoDB** credentials (hosts, replica set name, username/password if applicable).  
  ...
  - [Docker Compose instructions to spin up MongoDB replica sets](../connectors/mongodb/setup/local)
...
2. **Inside this folder**, create two files:
   ...
   - **`source.json`**: Contains connection settings for MongoDB (or other databases in the future). <br/>
...
Refer to [source configuration](../connectors/mongodb/config) for more details on `source.json`.
...
For more details on the  `state.json` configuration, refer the [state docs](../connectors/mongodb/config#statejson-configuration)
