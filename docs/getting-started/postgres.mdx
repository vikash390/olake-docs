...
OLake helps you replicate data from Postgres (and in the future, other databases) into local or S3-based data lakes using Parquet or Iceberg table formats. This tutorial walks you through every step of the setup—from creating necessary configuration files to running your first data sync.
...
- **Postgres** credentials (hosts, replica set name, username/password if applicable).  
  ...
  - [Docker Compose instructions to spin up Postgres replica sets](../connectors/postgres/docker-compose)
...
2. **Inside this folder**, create two files:
   ...
   - **`config.json`**: Contains connection settings for Postgres (or other databases in the future). <br/>
...
### 1.2 Example `config.json` (Postgres)
...
Refer to [source configuration](../connectors/postgres/config) for more details on `config.json`.

## Step 2: Generate a Catalog File

OLake needs to discover which collections (streams) exist in your Postgres. This step will create a `catalog.json` listing available streams, schemas, and default sync modes.
...
| `--config /mnt/config/config.json` | Tells OLake where to find your Postgres connection details. |
...
Now that you have `catalog.json`, it’s time to **sync** data from Postgres to your specified destination (local or S3).
...
| `--config /mnt/config/config.json`            | Postgres connection settings.                                               |
