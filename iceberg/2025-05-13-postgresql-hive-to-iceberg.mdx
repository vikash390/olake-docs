---
slug: postgresql-hive-to-iceberg
title: From PostgreSQL, Hive to Iceberg - What Apache Iceberg brings to Partitioning
...
image: /img/blog/cover/postgresql-hive-to-iceberg-cover.png
authors: [sandeep]
tags: [iceberg, postgres, hive]
---

![postgresql-hive-to-iceberg](/img/blog/cover/postgresql-hive-to-iceberg-cover.png)
...
![postgresql-hive-to-iceberg-1](/img/blog/2025/05/postgresql-hive-to-iceberg-1.png)
...
![postgresql-hive-to-iceberg-2](/img/blog/2025/05/postgresql-hive-to-iceberg-2.png)
...
![postgresql-hive-to-iceberg-3](/img/blog/2025/05/postgresql-hive-to-iceberg-3.png)
...
## Partitioning in PostgreSQL vs. Apache Hive vs. Apache Iceberg

Partitioning behaves differently across database systems. Let’s compare how PostgreSQL (a traditional relational database), Apache Hive (a SQL-on-Hadoop engine), and Apache Iceberg (a modern table format for data lakes) approach partitioning.

### Partitioning in PostgreSQL

PostgreSQL supports declarative table partitioning, which means you can create a table and partition it by `RANGE`, `LIST`, or `HASH` in your DDL. Internally, each partition is a child table. Postgres ensures that each row of the parent goes into exactly one partition based on a partition key. For example, you might do:
...
Here, orders is the partitioned table and `orders_2022`, `orders_2023` are partitions holding rows for their date ranges.

PostgreSQL’s partitioning is powerful but comes with some constraints. Notably, any **PRIMARY KEY or UNIQUE index on a partitioned table must include the partition key**. This is to ensure uniqueness across all partitions. For instance, if you partition by `order_date`, you cannot have a primary key on just id unless you include `order_date` too – otherwise two different partitions could have the same id value without conflict. Another limitation is that **global indexes** (an index spanning all partitions) are not supported. 

Instead, each partition has its own local index. When you query the table, Postgres will check the WHERE clause for the partition key and **prune** partitions that don’t apply. For example, a query `SELECT * FROM orders WHERE order_date = '2023-05-01'` will automatically target only the 2023 partition.

While Postgres automates partition pruning, you are responsible for managing partitions over time – e.g. creating new partitions for future dates. There are tools like `pg_partman` or features like declarative partitioning to help automate this, but it’s something to plan for. Also, if your queries don’t filter by the partition key, Postgres may have to scan all partitions, potentially hurting performance. 

In summary, in PostgreSQL partitioning works best when your partition key is part of most queries, and the system enforces some rules to maintain data integrity and performance. For example, if a table is partitioned by `signup_date`, a primary key like `(user_id, signup_date)` is valid because it scopes uniqueness within each partition. However, attempting to define a primary key only on `user_id` would fail, since PostgreSQL cannot guarantee uniqueness without searching every partition. These rules help optimize query execution and uphold consistency even as data scales across many partitions.
...
Let’s compare how PostgreSQL, Hive, and Iceberg partitioning stack up in real-world scenarios, focusing on performance, manageability, and use cases.
...
For Postgres, partitioning can boost performance on big tables, but since Postgres runs on a single node, its scale is more limited. Postgres might handle millions of rows efficiently with partitioning, but for billions or trillions of rows, a distributed system (like Hive/Iceberg on a cluster) is usually needed. 

Also, if a Postgres query can’t use partition pruning (e.g. no partition key in WHERE clause), it may actually be slower due to planning time checking many partitions. The bottom line: **Partitioning helps performance if aligned with query patterns**; Iceberg tends to maintain that benefit even as partitions grow very numerous by offloading work to its metadata, whereas Hive and Postgres might struggle beyond a certain partition count or skew.
...
Iceberg helps here by enabling on-the-fly aggregation (with query engines reading base detail data) and by offering table-level compaction to merge small files. Postgres, on the other hand, stores partitions as regular tables on disk – it generally handles storage internally, but very large partitions could bloat indexes or require periodic vacuuming.  
...
PostgreSQL is in the middle: once set up, the pruning is automatic, but **managing partitions** (especially time-based ones) might require scheduling jobs to create new partitions (e.g., one partition per month) and possibly retire old ones. There are extension tools to help, but it’s extra work. Also, if you ever need to change the partition key or strategy in Postgres, it’s a significant operation (you’d likely need to reorganize the table).
...
* **PostgreSQL** partitioning is good for *moderate-sized* datasets that still fit on one server’s storage and where low-latency OLTP or mixed workloads are needed. For instance, partitioning can help a fintech company’s PostgreSQL database manage a large transactions table by date – keeping recent hot data separate from old data. However, at some point (hundreds of millions or billions of rows), they might scale out to a data lake or sharded databases.  
